...
void tool() {
	...
	wstring getPath = fullpath;
	string pathMax( getPath.begin(), getPath.end() );
	if (!init) {
	...
	ImGui::InputText( "AddLogTitle", addLogTitle, sizeof( addLogTitle ) );

	if (ImGui::Button( "Grab", ImVec2( 100.0f, 100.0f ) )) {



		strcpy( buff, std::format( "{}\\{}.jpg", pathMax, file ).c_str() );


	}

	ImGui::Text( "%s", strlen( buff ) == 0 ? "getting path" : buff );

	ImGui::Text( "%d %d %d %d %.2f %.2f", region.x, region.y, region.width, region.height, mousePos.x, mousePos.y );

	static string generate = "";
	ImGui::InputTextMultiline( "###GENCODE", &generate, ImVec2( 300, 300 ), ImGuiInputTextFlags_Multiline );

	if (ImGui::Button( "Generate", ImVec2( 100.0f, 100.0f ) )) {

		generate = std::format(
			"Mat {} = imread( std::format( \"{{}}\\\\{}.jpg\", NPath ), IMREAD_COLOR ); \n"
			"getRoi(screen, {}, {}, {}, {}); \n"
			"pos = MatchingMethod({}); \n"
			"if (pos[0] > 0) {{ AU3_ControlClickByHandle( lpPlayerHWND, ldPlayerCrtlHWND, L\"Left\", 2, {},{} ); }} \n"
			"AddLog(\"{}\"); \n",
			file, file, region.x, region.y, region.width, region.height, file, mousePos.x, mousePos.y, addLogTitle
		).c_str();
	}
...
void snapshot() {
	...
	if (test && img) {
		...
		if (ImGui::IsItemHovered()) {
			if (cropState == 4 && ImGui::IsMouseClicked( ImGuiMouseButton_Right, false ))
				cropState = 0;
			if (cropState == 0 && ImGui::IsMouseClicked( ImGuiMouseButton_Left, false )) {
				...
				region.x		= s0.x;
				region.y		= s0.y;
				region.width	= s1.x - s0.x;
				region.height	= s1.y - s0.y;
			...
			else if (cropState == 3 && ImGui::IsMouseClicked( ImGuiMouseButton_Left, false )) {
				crop_max2   = ImGui::GetMousePos() - pos;
				cropState  = 0;
				cropDone   = true;

				auto min   = ImVec2( std::min( crop_min2.x, crop_max2.x ), std::min( crop_min2.y, crop_max2.y ) );
				auto max   = ImVec2( std::max( crop_min2.x, crop_max2.x ), std::max( crop_min2.y, crop_max2.y ) );
				auto uv0   = min / size;
				auto uv1   = max / size;
				auto s0    = uv0 * ImVec2( ( float )img->Width, ( float )img->Height );
				auto s1    = uv1 * ImVec2( ( float )img->Width, ( float )img->Height );
		...
		if (cropState == 3) {
			drawList->AddRect( pos + crop_min2, ImGui::GetMousePos(), 0xff0000ff );
		}
		else if (cropState == 4 && capDone) {
			drawList->AddRect( pos + crop_min2, pos + crop_max2, 0xff00ff00 );
